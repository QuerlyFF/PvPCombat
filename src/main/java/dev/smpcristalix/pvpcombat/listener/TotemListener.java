...
/** Лимит тотемов и Combat-ограничения источников без спама и потери ItemMeta. */
public final class TotemListener implements Listener {
    ...
    public TotemListener(CombatService combat, NoticeService notices, PvPCombatSettings settings) {
        this.combat = combat;
    ...
    public void onPickup(EntityPickupItemEvent event) {
        ...
        if (countTotems(player) + stack.getAmount() > settings.maxTotemsCarried()) {
            ...
            notices.warn(player, "totem-limit", "Лимит тотемов: " + settings.maxTotemsCarried());
            return;
        }
    ...
    public void onClick(InventoryClickEvent event) {
        ...
        if (combat.inCombat(player)
            ...
            notices.warn(player, "totem-container", "Во время Combat нельзя собирать тотемы из контейнеров.");
            return;
        }
        ...
        if (combat.inCombat(player) && !settings.allowExternalTotemSourcesInCombat()) {
            event.setCancelled(true);
            notices.warn(player, "totem-container", "Во время Combat нельзя брать тотемы из контейнеров.");
            return;
        }
    ...
    public void enforceLimit(Player player) {
        ...
        for (int slot = 0; slot < contents.length; slot++) {
            ...
            ItemStack droppedCopy = item.clone();
        ...
        if (!overflow.isEmpty()) {
            notices.warn(
                    player,
                    "totem-limit",
                    "Лимит тотемов: " + maximum + ". Лишние тотемы выброшены рядом."
            );
        }
    }
